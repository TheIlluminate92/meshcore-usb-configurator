...
Bug hunt: fixed nonfinite battery-input crash, channels-only/empty shared-scope expansion, malformed-library-metadata crash, update-profile channel-scope expansion/missing-field loss and batch stale checks for unchanged included channels. Local BLE package replaced by extracted bluetooth_libs copy preferred by launch.py; old vendor copy remains inaccessible. All 60 tests now pass with PYTHONPATH=bluetooth_libs;vendor. USB read 22/40/no errors; Windows BLE scanner still unavailable/off. No hardware writes.

Fleet workflow: local SQLite identity/run history with JSON profiles; compare and preview views; profile-linked prefix/start naming; per-device progress; CSV history export; read-only verification after user-attested restart against saved expected values. Interrupted work never resumes automatically. Main pending edits have discard prompts. Hardware write/BLE/power-cycle validation remains pending.
