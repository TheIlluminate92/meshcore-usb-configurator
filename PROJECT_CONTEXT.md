...
Current implementation supports eight editable fields and read-only snapshots of additional reported data. Single-device USB reads were demonstrated on hardware. A subsequent write attempt failed opening COM4; a cleanup race was fixed and tested with the actual asynchronous serial transport over a loopback port. Successful hardware writing and persistence after restart have not yet been confirmed.

Next development: map full reference structure to supported read/write/verify commands; introduce appropriately constrained controls and grouped settings; add channels without copying private identity; validate the write path on the T114; then build a device-by-device batch queue with individual reports and naming rules.
